/* define system option to specify the order of date */
option datestyle=dmy;

/* Create a reference to the file */
filename retail "/folders/myfolders/raw_file/online_retail.csv";

proc import file=retail out=online_retail dbms=csv replace;
run;
...
/* get the product with max quantity in each country */
proc sql;
create table maximum_product_country_wise_1 as
...
proc sort data=maximum_product_country_wise_1;
by product_id;
run;

/* merge with product table to get the product names */
data maximum_product_country_wise;
merge maximum_product_country_wise_1 (in=a) product_detail(in=b);
by product_id;
if a;
run;
